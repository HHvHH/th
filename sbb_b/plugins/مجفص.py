...
async def spam_function(event, RR7PP, sbb_b, sleeptimem, sleeptimet, DelaySpam=False):
    ...
    if len(sbb_b) == 2:
        ...
        for _ in range(counter):
            ...
            await _sbb_butils.unsavegif(event, RR7PP)
...
async def spammer(event):
    ...
    await spam_function(event, RR7PP, sbb_b, sleeptimem, sleeptimet)
...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, sbb_b, sleeptimem, sleeptimet, DelaySpam=True)


@sbb_b.ar_cmd(pattern="تكرار الملصق$")
